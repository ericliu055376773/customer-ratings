...
// 改回靜態載入，避免 Vite 動態打包器 (Optimizer) 卡死
...
// ==========================================
// 2. Firebase 安全初始化 (放置於頂層，加速 Vite 解析)
// ==========================================
let app: any = null;
let auth: any = null;
let db: any = null;
...
try {
  ...
  console.warn('Firebase 初始化略過 (將以純 UI 模式執行):', error);
}
...
function RatingApp() {
  ...
  useEffect(() => {
    ...
    signInAnonymously(auth).catch(() => console.warn('匿名登入跳過'));
